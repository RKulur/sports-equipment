...
export default function Products() {
  ...
  useEffect(() => {
    ...
    Axios.get('http://localhost:7000/api/user/products', {
      headers: {
        'auth-token': JSON.parse(token)
      }
    }).then((res) => {
      console.log(res.data.products,222222222)
              ...
              products.map((product, index) => (
                <SwiperSlide key={index}>
                  <div className="box">
                    ...
                    <img src={product?.image} className='object-contain w-full ' alt="no image" />
                    <h3>{product?.name}</h3>
